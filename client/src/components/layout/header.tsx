...
export function Header({ title = 'Command Center', onRefresh, refreshing }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    // Global Command Palette Shortcut Listener (⌘K or Ctrl+K)
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs transition-all hover:opacity-80"
          style={{ background: 'var(--secondary)', color: 'var(--muted-foreground)', border: '1px solid var(--border)' }}
          title="Search telemetry and commands (⌘K)"
        >
          <Search className="w-3.5 h-3.5" />
          <span className="hidden md:inline">Search...</span>
          <kbd className="text-[10px] px-1 py-0.5 rounded bg-muted font-mono-data border border-border">⌘K</kbd>
        </button>

        {/* Live Telemetry Refresh Indicator */}
        {onRefresh && (
          <button
            onClick={handleRefreshClick}
            className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs hover:opacity-80"
            style={{ background: 'var(--secondary)', color: 'var(--muted-foreground)', border: '1px solid var(--border)' }}
            title="Fetch latest telemetry stream"
          >
        ...
        <Link
          href="/profile"
          className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 bg-gradient-to-br from-rose-500 to-indigo-500 shadow-md"
          title="View Profile"
        >
          SA
        </Link>
        ...
        <Drawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title="Active Notifications">
            ...
            <div className="fixed inset-0 z-[110] md:hidden">
              ...
              >
                <div className="flex items-center justify-between pb-4 border-b" style={{ borderColor: 'var(--sidebar-border)' }}>
                  ...
                  <button onClick={() => setMobileMenuOpen(false)} className="p-1 rounded-lg text-muted-foreground hover:text-foreground">
                    <X className="w-4 h-4" />
                  </button>
                </div>

                <nav className="flex-1 py-4 space-y-1 overflow-y-auto">
                  <p className="text-xs font-semibold tracking-widest uppercase px-3 mb-2 text-muted-foreground">Workspace</p>
                  ...
                  <p className="text-xs font-semibold tracking-widest uppercase px-3 mt-5 mb-2 text-muted-foreground">Account</p>
